...
#data = read.csv('G:/TLSdata/TLS_total_data/table/CD4_T_clone_id_group.csv',header = TRUE)
...
dataframe_hm = data.frame(matrix(ncol = 0, nrow = 13))###得第一次有6行
n=c()
for (j in 1:13){
  for (k in 1:13){
    c_name = names[j]
...
ggsave("G:/TLSdata/TLS_total_data/figures/TCR_CD4/CD4_T_clone_share_hm.pdf",plot = CD4_T_clone_share_hm,  width = 6.6, height = 6)
...
table$r = sqrt(table$total)/30 #盘子大小
...
ggsave("G:/TLSdata/TLS_total_data/figures/TCR_CD4/CD4_clone_share_scatter.pdf",plot = scatter_CD4,  width = 4.5, height = 4)
