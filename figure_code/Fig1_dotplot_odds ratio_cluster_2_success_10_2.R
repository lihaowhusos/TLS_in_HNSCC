...
## load data --------------------------------------
scrna_cell_tbl <- read.csv('G:/TLSdata/TLS_total_data/Results/cell_count/adata.obs.csv',header = TRUE)
...
scrna_cell_tbl <- scrna_cell_tbl %>% 
  filter(tissue_type == "CD45") %>% 
  filter(!(cluster_2 %in% c("Fibroblast", "Fibroblast_Pericytes", "Endothelial_cell", "Endothelial_Lymphatic_endothelial_cell","Fibroblast","Fibroblast_Pericytes","Cancer_cell","Endothelial_cell")))%>% 
...
####### re-group #################
##  [1] B   B_Plasma_cell  Mast_cell    Myeloid_cDC    Myeloid_LAMP3_DC   Myeloid_Mono_Macro  Myeloid_pDC  Neutrophil    T_CD4_T      T_CD4_Treg     T_CD8_T    T_Cycling_T    T_NK_NKT 
custom_order <- c("T_CD4_T", 
                  "T_CD8_T",
                  "T_CD4_Treg", 
                  "T_NK_NKT",
                  "T_Cycling_T",
...
ggsave("G:/TLSdata/TLS_total_data/figures/dotplot_hotmap/cluster_2_dot_plot.pdf",plot = p_scrna_odds_ratio)
