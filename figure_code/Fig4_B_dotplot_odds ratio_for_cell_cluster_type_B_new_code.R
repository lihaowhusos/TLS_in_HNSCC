...
cell_data <- read.csv('XXX/B_Plasma_for_seurat_metadata.csv', header = TRUE)
...
p_sig_anno <- odds_result %>% 
  distinct(contrast_name, contrast_var, .keep_all = TRUE) %>% 
  ggplot() +
...
p_cell_anno <- odds_result %>% 
  mutate(facet_var = "") %>% 
  distinct(cell_type, facet_var, .keep_all = TRUE) %>% 
  ggplot() +
