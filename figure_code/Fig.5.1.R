...
count_net <- count_net[!grepl('Plasma|Cycling', count_net$SOURCE) & !grepl('Plasma|Cycling', count_net$TARGET), ]
...
# Extract and visualize the cellchat network weights
...
# Define the netVisual_circle function
netVisual_circle <- function(net, color.use = NULL, title.name = NULL, sources.use = NULL, targets.use = NULL, idents.use = NULL, remove.isolate = FALSE, top = 1,
  ...
  if (!is.null(sources.use) | !is.null(targets.use) | !is.null(idents.use)) {
    ...
    if (!is.null(sources.use)) df.net <- subset(df.net, source %in% sources.use)
    if (!is.null(targets.use)) df.net <- subset(df.net, target %in% targets.use)
    if (!is.null(idents.use)) df.net <- filter(df.net, (source %in% idents.use) | (target %in% idents.use))
  ...
  if (remove.isolate) {
    idx <- which(Matrix::rowSums(net) == 0 & Matrix::colSums(net) == 0)
    net <- net[-idx, -idx]
  }
  ...
  if (is.null(color.use)) color.use <- scPalette(length(igraph::V(g)))
  ...
  if (is.null(edge.weight.max)) edge.weight.max <- max(igraph::E(g)$weight)
  ...
  if (!is.null(title.name)) text(0, 1.5, title.name, cex = 1.1)
  gg <- recordPlot()
  return(gg)
}