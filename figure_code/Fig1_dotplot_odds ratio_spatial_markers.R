...
raw_table <- read.csv('path/markers_for_R.csv',header = TRUE)
...
gene_dotplot_odds_ratio <-function(table,gene){
  ...
  # Prepare cell_id column and remove first column (ID column)
  cell_data_tbl = raw_table
  
  
  cell_data_tbl$cell_id <- cell_data_tbl[, 1]
  cell_data_tbl <- cell_data_tbl[, -1]
  
  ## Join and filter data
  cell_data_tbl[,"cell_type"] <- cell_data_tbl[,gene]
  cell_data_tbl <- cell_data_tbl %>% 
    mutate(cell_type = cluster_1) %>% 
    mutate(
      tumor_category = group,            # Renaming group to tumor_category
      cell_identifier = cell_id          # Renaming barcode to cell_identifier
    )
...
return(p_cell_glm_result) 
}

p_CD20 <- gene_dotplot_odds_ratio(raw_table,"CD20")

p_CD8 <- gene_dotplot_odds_ratio(raw_table,"CD8")

p_CD4 <- gene_dotplot_odds_ratio(raw_table,"CD4")

p_TCF1 <- gene_dotplot_odds_ratio(raw_table,"TCF1")

p_CD23 <- gene_dotplot_odds_ratio(raw_table,"CD23")

p_total <- grid.arrange(p_CD20, p_CD8, p_CD4, p_TCF1, p_CD23, nrow = 1)

ggsave("XXX.pdf",plot = p_total, width = 15, height = 8)
